    ...
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
        <div>
            <div style="margin-bottom: 24px;">
                <p style="margin: 0 0 8px; color: #667085; font-size: 12px; text-transform: uppercase; font-weight: 700;">Catégorie</p>
                <p style="margin: 0; color: #172033; font-size: 16px;">{{ $produit->categorieProduit->nom }}</p>
            </div>

            @if ($produit->description)
                <div style="margin-bottom: 24px;">
                    <p style="margin: 0 0 8px; color: #667085; font-size: 12px; text-transform: uppercase; font-weight: 700;">Description</p>
                    <p style="margin: 0; color: #172033; font-size: 14px; line-height: 1.6;">{{ $produit->description }}</p>
                </div>
            @endif

            <div style="margin-bottom: 24px;">
                <p style="margin: 0 0 8px; color: #667085; font-size: 12px; text-transform: uppercase; font-weight: 700;">Prix unitaire</p>
                <p style="margin: 0; color: #172033; font-size: 16px; font-weight: 700;">{{ number_format($produit->prix, 2, ',', ' ') }} €</p>
            </div>

            <div style="margin-bottom: 24px;">
                <p style="margin: 0 0 8px; color: #667085; font-size: 12px; text-transform: uppercase; font-weight: 700;">Créé le</p>
                <p style="margin: 0; color: #172033; font-size: 14px;">{{ $produit->created_at->format('d/m/Y à H:i') }}</p>
            </div>

            @if ($produit->updated_at->ne($produit->created_at))
                <div style="margin-bottom: 24px;">
                    <p style="margin: 0 0 8px; color: #667085; font-size: 12px; text-transform: uppercase; font-weight: 700;">Mis à jour le</p>
                    <p style="margin: 0; color: #172033; font-size: 14px;">{{ $produit->updated_at->format('d/m/Y à H:i') }}</p>
                </div>
            @endif
        </div>
